 ...
 */
class ScaleSpace3d : public Plugin
{
public:
    ScaleSpace3d()
          ...
          sampleRate(getSampleRate())
    {
		...
		z_range_min = controlLimits[kParameterZ].first;
		z_range_max = controlLimits[kParameterZ].second;
...
protected:
    ...
    */
    ...
    {
        return "3 Axis MTS-ESP Scale Space Explorer.";
    }
    ...
    */
    ...
    {
        return "https://github.com/eventual-recluse/ScaleSpace-3D";
    }
    ...
    */
    ...
    void loadScl(Tunings::Tuning & tn, const char* value)
    {
		...
		if (filename.endsWith(".scl"))
		{
			...
			{   auto s = Tunings::readSCLFile(value);
				...
				//d_stdout("ScaleSpace3D: tuning set to %s", value);
			}
			catch (const std::exception& e)
			{
				tn = Tunings::Tuning();
				d_stdout("ScaleSpace3D:Exception when setting tuning");
				d_stdout(e.what());
			}
		}
		else
		{
			...
			tn = Tunings::Tuning(s, k);
			//d_stdout("ScaleSpace3D: tuning scl reset");
		}
	}
	...
	void loadKbm(Tunings::Tuning & tn, const char* value)
	{
		...
		else
		{
			...
			tn = Tunings::Tuning(s, k);
			//d_stdout("ScaleSpace3D: tuning kbm reset");
		}
	}
	...
	void saveScale(const char* value)
	{
		...
		if (filename_path.isNotEmpty())
		{
			...
			if (savefileSCL.is_open())
			{
				...
				textStream << "!" << "\n";
				textStream << scl_basename << " exported from ScaleSpace3D" << "\n";
				textStream << " 128" << "\n";
				textStream << "!" << "\n";
			...
			if (savefileKBM.is_open())
			{
				...
				textStream << "! " << "Exported from ScaleSpace3D" << "\n";
				textStream << "! " << "Size:" << "\n";
				textStream << "0" << "\n";
    ...
    */
    void run(const float** inputs, float** outputs, uint32_t frames) override
    {
		...
		{
			double freq1_c = tuning1.frequencyForMidiNote(i) * (0.5f - (fParameters[kParameterX] / x_size)) * (0.5f + (fParameters[kParameterY] / y_size)) * (0.5f - (fParameters[kParameterZ] / z_size));
			double freq2_c = tuning2.frequencyForMidiNote(i) * (0.5f + (fParameters[kParameterX] / x_size)) * (0.5f + (fParameters[kParameterY] / y_size)) * (0.5f - (fParameters[kParameterZ] / z_size));
			double freq3_c = tuning3.frequencyForMidiNote(i) * (0.5f - (fParameters[kParameterX] / x_size)) * (0.5f - (fParameters[kParameterY] / y_size)) * (0.5f - (fParameters[kParameterZ] / z_size));
			double freq4_c = tuning4.frequencyForMidiNote(i) * (0.5f + (fParameters[kParameterX] / x_size)) * (0.5f - (fParameters[kParameterY] / y_size)) * (0.5f - (fParameters[kParameterZ] / z_size));
			
			double freq5_c = tuning5.frequencyForMidiNote(i) * (0.5f - (fParameters[kParameterX] / x_size)) * (0.5f + (fParameters[kParameterY] / y_size)) * (0.5f + (fParameters[kParameterZ] / z_size));
			double freq6_c = tuning6.frequencyForMidiNote(i) * (0.5f + (fParameters[kParameterX] / x_size)) * (0.5f + (fParameters[kParameterY] / y_size)) * (0.5f + (fParameters[kParameterZ] / z_size));
			double freq7_c = tuning7.frequencyForMidiNote(i) * (0.5f - (fParameters[kParameterX] / x_size)) * (0.5f - (fParameters[kParameterY] / y_size)) * (0.5f + (fParameters[kParameterZ] / z_size));
			double freq8_c = tuning8.frequencyForMidiNote(i) * (0.5f + (fParameters[kParameterX] / x_size)) * (0.5f - (fParameters[kParameterY] / y_size)) * (0.5f + (fParameters[kParameterZ] / z_size));
			
			target_frequencies_in_hz[i] = freq1_c + freq2_c + freq3_c + freq4_c + freq5_c + freq6_c + freq7_c + freq8_c;
...
private:
    ...
    */
    DISTRHO_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(ScaleSpace3d)
};
